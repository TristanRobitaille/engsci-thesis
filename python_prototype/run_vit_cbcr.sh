#!/bin/bash

# First set

python3 /home/trobitaille/engsci-thesis/python_prototype/main_vision_transformer.py \
--clip_length_s=30 \
--dataset_resample_strategy=auto \
--dataset_resample_replacement=True \
--batch_size=16 \
--learning_rate=3e-3 \
--num_epochs=20 \
--input_channel='EEG Pz-LER' \
--num_clips=115000 \
--embedding_depth=128 \
--num_layers=16 \
--num_heads=8 \
--mlp_dim=32 \
--input_dataset="/mnt/data/tristan/engsci_thesis_python_prototype_data/SS3_EDF_Tensorized_6-stg_30s" &

python3 /home/trobitaille/engsci-thesis/python_prototype/main_vision_transformer.py \
--clip_length_s=30 \
--dataset_resample_strategy=auto \
--dataset_resample_replacement=True \
--batch_size=16 \
--learning_rate=3e-3 \
--num_epochs=20 \
--input_channel='EEG Pz-LER' \
--num_clips=115000 \
--embedding_depth=128 \
--num_layers=16 \
--num_heads=8 \
--mlp_dim=32 \
--input_dataset="/mnt/data/tristan/engsci_thesis_python_prototype_data/SS3_EDF_Tensorized_5-stg_30s" &


# Second set (runs in parallel of set #1)
wait
