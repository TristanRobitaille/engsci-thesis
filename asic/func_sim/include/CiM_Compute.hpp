    ...
    public:
        ...
        void int_res_write(float data, uint32_t addr, DATA_WIDTH data_width) {
            ...
            if (addr < CIM_INT_RES_BANK_SIZE_NUM_WORD) {
                int_res_0[addr] = data;
                if (data_width == DOUBLE_WIDTH) { int_res_0[addr + 1] = data; }
            } else if (addr < 2 * CIM_INT_RES_BANK_SIZE_NUM_WORD) {
                int_res_1[addr - CIM_INT_RES_BANK_SIZE_NUM_WORD] = data;
                if (data_width == DOUBLE_WIDTH) { int_res_1[addr - CIM_INT_RES_BANK_SIZE_NUM_WORD + 1] = data; }
            } else if (addr < 3 * CIM_INT_RES_BANK_SIZE_NUM_WORD) {
                int_res_2[addr - 2*CIM_INT_RES_BANK_SIZE_NUM_WORD] = data;
                if (data_width == DOUBLE_WIDTH) { int_res_2[addr - 2*CIM_INT_RES_BANK_SIZE_NUM_WORD + 1] = data; }
            } else if (addr < 4 * CIM_INT_RES_BANK_SIZE_NUM_WORD) {
                int_res_3[addr - 3*CIM_INT_RES_BANK_SIZE_NUM_WORD] = data;
                if (data_width == DOUBLE_WIDTH) { int_res_3[addr - 3*CIM_INT_RES_BANK_SIZE_NUM_WORD + 1] = data; }
...
#elif CENTRALIZED_ARCH
            verify_result(MEAN, static_cast<float>(compute_temp_fp_1), int_res_read, input_addr, EMB_DEPTH, DOUBLE_WIDTH);
#endif
            // Variance
...
#elif CENTRALIZED_ARCH
            verify_result(VARIANCE, static_cast<float> (compute_temp_fp_2), int_res_read, input_addr, EMB_DEPTH, DOUBLE_WIDTH);
#endif
        ...
        void LAYERNORM_2ND_HALF(uint32_t input_addr, uint32_t gamma_addr, uint32_t beta_addr, uint16_t num_rows) {
            ...
            for (uint16_t i = 0; i < num_rows; i++) { // Go through each rows in given column
                compute_temp_fp_1 = comp_fx_t { static_cast<in1_storage_fx_t>(int_res_read(input_addr + i*DOUBLE_WIDTH*EMB_DEPTH)) }; // Go down column
                float result = static_cast<float>(compute_temp_fp_2 * compute_temp_fp_1 + compute_temp_fp_3);
        ...
        uint16_t find_softmax_argmax(uint32_t base_addr) {
            ...
            for (uint32_t i=0; i<NUM_SLEEP_STAGES; i++) {
                cout << int_res_read(base_addr + DOUBLE_WIDTH*i) << " ";
                if (int_res_read(base_addr + DOUBLE_WIDTH*i) > softmax_max) {
                    softmax_max = int_res_read(base_addr + DOUBLE_WIDTH*i);
                    argmax_index = i;
                }
            }
