...
/*----- MAP -----*/
const std::map<DATA, uint32_t> mem_map = {
    {EEG_INPUT_MEM,             0}, // Size: PATCH_LEN*NUM_PATCHES
    {PATCH_MEM,                 SINGLE_WIDTH*NUM_PATCHES*PATCH_LEN + PATCH_LEN}, // Size: DOUBLE_WIDTH*NUM_PATCHES*EMB_DEPTH
    {CLASS_TOKEN_MEM,           SINGLE_WIDTH*NUM_PATCHES*PATCH_LEN}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {POS_EMB_MEM,               0}, // Need to protect - Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_LN1_MEM,               20000 /* Somewhere far */}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_Q_MEM,                 2*(NUM_PATCHES+1)*EMB_DEPTH}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_K_MEM,                 2*(NUM_PATCHES+1)*EMB_DEPTH+(NUM_PATCHES+1)*EMB_DEPTH}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_V_MEM,                 (NUM_PATCHES+1)*EMB_DEPTH}, // Need to protect positional embedding - Size: (NUM_PATCHES+1)*EMB_DEPTH
    {ENC_QK_T_MEM,              4*(NUM_PATCHES+1)*EMB_DEPTH}, // Size: EMB_DEPTH*(NUM_PATCHES+1)*(NUM_PATCHES+1)
    {ENC_V_MULT_MEM,            4*(NUM_PATCHES+1)*EMB_DEPTH + NUM_HEADS*(NUM_PATCHES+1)*(NUM_PATCHES+1)}, // Size: (NUM_PATCHES+1)*EMB_DEPTH
    {ENC_MHSA_OUT_MEM,          (NUM_PATCHES+1)*EMB_DEPTH}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_LN2_MEM,               0}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*EMB_DEPTH
    {ENC_MLP_DENSE1_MEM,        EMB_DEPTH*EMB_DEPTH}, // Size: DOUBLE_WIDTH*(NUM_PATCHES+1)*MLP_DIM
    {ENC_MLP_OUT_MEM,           0}, // Size: DOUBLE_WIDTH*EMB_DEPTH
    {ENC_LN3_MEM,               EMB_DEPTH}, // Size: DOUBLE_WIDTH*EMB_DEPTH
    {MLP_HEAD_DENSE_1_OUT_MEM,  0}, // Size: DOUBLE_WIDTH*MLP_DIM
    {MLP_HEAD_DENSE_2_OUT_MEM,  MLP_DIM}, // Size: DOUBLE_WIDTH*NUM_SLEEP_STAGES
    {SOFTMAX_AVG_SUM_MEM,       0}, // Size: DOUBLE_WIDTH*NUM_SLEEP_STAGES
    {PREV_SOFTMAX_OUTPUT_MEM,   NUM_PARAMS_BANKS*CIM_PARAMS_BANK_SIZE_NUM_WORD - NUM_SLEEP_STAGES*(NUM_SAMPLES_OUT_AVG-1)} // Store in params memory to allow power gating int res memory
};
