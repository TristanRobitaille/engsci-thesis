...
const std::map<DATA, uint32_t> mem_map = {
    ...
    {ENC_QK_T_MEM,              2*(NUM_PATCHES+1)*EMB_DEPTH + 2*(NUM_PATCHES+1)*EMB_DEPTH}, // Size: EMB_DEPTH*(NUM_PATCHES+1)*(NUM_PATCHES+1)
    {ENC_V_MULT_MEM,            2*(NUM_PATCHES+1)*EMB_DEPTH + 2*(NUM_PATCHES+1)*EMB_DEPTH + NUM_HEADS*(NUM_PATCHES+1)*(NUM_PATCHES+1)}, // Size: (NUM_PATCHES+1)*EMB_DEPTH
    ...
    {PREV_SOFTMAX_OUTPUT_MEM,   NUM_INT_RES_BANKS*CIM_INT_RES_BANK_SIZE_NUM_WORD - NUM_SLEEP_STAGES*(NUM_SAMPLES_OUT_AVG-1)}
};
