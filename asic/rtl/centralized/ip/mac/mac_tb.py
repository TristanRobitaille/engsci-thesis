...
#----- FUNCTIONS -----#
async def test_MAC(dut, activation, direction, matrix_width, param_type, int_res_width, int_res_format, param_format):
    # Prepare MAC
    expected_result = 0
    compute_temp_fp = 0
    start_addr_1 = random.randint(MAX_LEN*MAX_LEN, const.CIM_INT_RES_BANK_SIZE_NUM_WORD*const.CIM_INT_RES_NUM_BANKS - MAX_LEN*MAX_LEN - 1) # Starts at MAX_LEN to make it easier to correct for overlap
    start_addr_2 = random.randint(MAX_LEN*MAX_LEN, const.CIM_PARAMS_BANK_SIZE_NUM_WORD*const.CIM_PARAMS_NUM_BANKS - MAX_LEN*MAX_LEN - 1)

    # Fix address if there's an overlap
    if ((start_addr_1+MAX_LEN*MAX_LEN) > start_addr_2): start_addr_1 = start_addr_2 - MAX_LEN*MAX_LEN
    elif ((start_addr_2+MAX_LEN*MAX_LEN) > start_addr_1): start_addr_2 = start_addr_1 - MAX_LEN*MAX_LEN
    ...
    dut.mac_direction.value = direction.value
    dut.mac_matrix_width.value = matrix_width
    ...
    for i in range(MAX_LEN):
        ...
        if (param_type == const.MACParamType.INTERMEDIATE_RES):
            if direction == const.MACDirection.HORIZONTAL: addr2 = start_addr_2+i
            else: addr2 = start_addr_2+i*matrix_width
            await utilities.write_one_word_cent(dut=dut, addr=addr2, data=in_2, device="int_res", data_width=int_res_width, data_format=int_res_format)
        elif (param_type == const.MACParamType.MODEL_PARAM): await utilities.write_one_word_cent(dut=dut, addr=start_addr_2+i, data=in_2, device="params", data_format=param_format, data_width=const.DataWidth.SINGLE_WIDTH)
...
async def MAC_test(dut):
    ...
    for activation in const.ActivationType:
        for mac_param_type in const.MACParamType:
            for direction in const.MACDirection:
                for data_width in const.DataWidth:
                    for int_res_format in [const.FxFormatIntRes.INT_RES_SW_FX_4_X, const.FxFormatIntRes.INT_RES_SW_FX_5_X, const.FxFormatIntRes.INT_RES_SW_FX_6_X]:
                        for param_format in [const.FxFormatParams.PARAMS_FX_4_X, const.FxFormatParams.PARAMS_FX_5_X]:
                            ...
                            for _ in range(NUM_TESTS): await test_MAC(dut,
                                                                      direction=direction,
                                                                      matrix_width=utilities.EMB_DEPTH,
